...
assert(
  html.includes('id="schematicTakeoffDialog"')
    ...
    && html.includes('id="schematicTakeoffClearQuestionsButton"')
    && html.includes('data-schematic-tool="manual"')
    && html.includes('id="schematicTakeoffStartSortButton"')
    && html.includes('id="schematicTakeoffReviewNextButton"')
    && html.includes('id="schematicTakeoffOpenOrderButton"')
    ...
    && /function\s+beginSchematicTakeoffManualPlacement\s*\(/.test(app)
    && /function\s+clearAllSchematicTakeoffQuestionMarks\s*\(/.test(app)
    && /function\s+setSchematicTakeoffReviewStep\s*\(/.test(app)
    && /function\s+openSchematicTakeoffManualEditor\s*\(/.test(app)
    ...
    && /function\s+schematicTakeoffTextGuidedSymbolType\s*\(/.test(app)
    && /function\s+schematicTakeoffDetectTextGuidedSymbols\s*\(/.test(app)
    && /classifiedGroups:\s*\[/.test(app)
    && app.includes('return "Pump"')
    && app.includes('return "Chiller"')
    && app.includes('return "Heat exchanger"')
    && app.includes('return "Temperature sensor"')
    && app.includes('return "Mag flow meter"')
